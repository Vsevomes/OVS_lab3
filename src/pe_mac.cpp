...
pe_mac::pe_mac() : pe_mac("nc") {};

pe_mac::pe_mac(sc_module_name nm)
    : sc_module(nm)
{
...
void pe_mac::load_data()
{
    while (1)
    {
        ...
        std::cout << "[pe_mac] Начало загрузки данных..." << std::endl;
        ...
        std::cout << "[pe_mac] Данные загружены, переход к вычислениям." << std::endl;
...
void pe_mac::process()
{
    while (1)
    {
        ...
        std::cout << "[pe_mac] Выполнение операций MAC..." << std::endl;

        accumulator = 0;
        while (!weight_queue.empty() && !value_queue.empty())
        {
            
            for (size_t i = 0; i < pe_mul_size; ++i)
            {
        ...
        // std::cout << "[pe_mac] Результат вычислений = " << accumulator << std::endl;

        state = RESULT;
    }
}

void pe_mac::write_result()
{
    while (1)
    {
        while (state != RESULT)
        ...
        std::cout << "[pe_mac] Запись результата в память..." << std::endl;
